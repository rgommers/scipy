...
The following functions from `scipy.misc` have been deprecated:
``who``, ``source``, ``comb``, ``factorial``, ``factorial2`` and
``factorialk``.  All those functions are simple aliases for numpy or
