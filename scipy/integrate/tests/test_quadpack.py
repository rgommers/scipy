...
try:
    ...
    cmd = "gcc -shared -o " + folder + "testlib.so -fPIC " + folder + "testlib.c"
...
class TestMultivariateCtypesQuad(TestCase): #Tests only work on Linux OS
    @dec.skipif(_ctypes_missing or _ctypes_multivariate_fail, msg="Compiling test functions failed")
    def setUp(self):
        fileext = '.so'
        dll_name = "testlib"
        file = os.path.dirname(os.path.abspath(__file__)) + os.path.sep + dll_name + fileext
        self.lib = ctypes.CDLL(file)
        restype = ctypes.c_double
        argtypes = (ctypes.c_int,ctypes.c_double)
        for name in ['typical','indefinite','sin2']:
            func = getattr(self.lib, name)
            func.restype = restype
            func.argtypes = argtypes
    ...
    def test_typical(self):
        ...
        assert_quad(quad(self.lib.typical,0,pi,(2,1.8)), 0.30614353532540296487)

    @dec.skipif(_ctypes_missing or _ctypes_multivariate_fail, msg="Compiling test functions failed")
    def test_indefinite(self):
        # 2) Infinite integration limits --- Euler's constant
        assert_quad(quad(self.lib.indefinite,0,Inf), 0.577215664901532860606512)

    @dec.skipif(_ctypes_missing or _ctypes_multivariate_fail, msg="Compiling test functions failed")
    def test_threadsafety(self):
        # 3) Ensure multivariate ctypes are threadsafe
        def threadsafety(y):
            return y + quad(self.lib.sin2, 0, 1)[0]
    ...
    def test_improvement(self):
        ...
        start = time.time()
        for i in xrange(100):
            quad(self.lib.sin2, 0, 100)
        fast = time.time() - start
        start = time.time()
        for i in xrange(100):
            quad(math.sin, 0, 100)
        slow = time.time() - start
        assert_(fast < 0.5*slow, (fast, slow)) # 2+ times faster speeds generated by trivial ctypes function
    ...
    def test_improvement2(self):
        ...
        start = time.time()
        for i in xrange(20):
            quad(self.lib.indefinite, 0, 100)
        fast = time.time() - start
        start = time.time()
        for i in xrange(20):
            quad(myfunc, 0, 100)
        slow = time.time() - start
        assert_(fast < 0.05*slow, (fast, slow)) # 20+ times faster speeds generated by nontrivial ctypes function (single variable)

class TestMultivariateCtypesQuad(TestCase):
    # Currently only functional on linux
    @dec.skipif(_ctypes_missing or _ctypes_multivariate_fail, msg="Compiling test functions failed")
    def setUp(self):
        fileext = '.so'
        dll_name = "test_multivariate"
        file = os.path.dirname(os.path.abspath(__file__)) + os.path.sep + dll_name + fileext
        self.lib = ctypes.CDLL(file)
        restype = ctypes.c_double
        argtypes = (ctypes.c_int,ctypes.c_double)
    ...
    def test_improvement(self):
        ...
        for i in xrange(100):
            quad(math.sin, 0, 100)
        slow = time.time() - start
        assert_(fast < .666 * slow, (fast, slow))  # >1.5x faster speeds generated by trivial ctypes function
