...
class TestMultivariateCtypesQuad(TestCase):
    ...
    def test_improvement(self):
        ...
        start = time.time()
        for i in xrange(100):
            quad(self.lib._multivariate_sin, 0, 100)
        fast = time.time() - start
        start = time.time()
        for i in xrange(100):
            quad(math.sin, 0, 100)
        slow = time.time() - start
        # >1.5x faster speeds generated by trivial ctypes function
    ...
    def test_improvement2(self):
        ...
        for i in xrange(20):
            quad(myfunc, 0, 100)
        slow = time.time() - start
        # ~20+ times faster speeds generated by nontrivial ctypes
        # function (single variable)
        assert_(fast < 0.05*slow, (fast, slow))
