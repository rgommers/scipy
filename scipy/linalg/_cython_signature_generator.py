...
# These are needed because LAPACK 3.10.0 rewrote them in free-form Fortran
# using the constants from ``la_constants``, which crackfortran cannot resolve
# (it reports every ``real(wp)``/``complex(wp)`` as single precision).  The
# argument names are those of the oldest supported LAPACK, except for zdrot,
# where they are the ones SciPy has always used (LAPACK 3.9.1 renamed CX/CY to
# ZX/ZY, but renaming them here would break downstream keyword arguments).
blas_manual_wrappers = '''void crotg(c *ca, c *cb, s *c, c *s)
