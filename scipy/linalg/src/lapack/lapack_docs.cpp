...
namespace lapack {
    namespace capi {
         ...
         *
         * The templates assemble their result in a `std::string`, so the allocation can throw.
         * This is declared `noexcept` because it is called straight from a getset slot; the
         * `catch` turns a `std::bad_alloc` into a `MemoryError` rather than `std::terminate`.
