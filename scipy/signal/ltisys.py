...
from numpy import (r_, eye, real, atleast_1d, atleast_2d, poly,
                   squeeze, diag, asarray, imag, newaxis, hstack, sqrt,
                   conj, product, zeros, array, dot, transpose, ones,
                   zeros_like)
...
def lsim2(system, U=None, T=None, X0=None, **kwargs):
    ...
    if U is not None:
        ...
        def fprime(x, t, sys, ufunc):
            ...
            return dot(sys.A, x) + squeeze(dot(sys.B, np.nan_to_num(ufunc([t]))))
...
def _cast_to_array_dtype(in1, in2):
    ...
    """
    if np.issubdtype(in2.dtype, np.float):
...
def lsim(system, U, T, X0=None, interp=1):
    ...
    vt = transpose(v)
    vti = linalg.inv(vt)
    GT = dot(dot(vti, diag(np.exp(dt * lam))), vt)
