...
class TestAnderson(TestCase):
    ...
    def test_gumbel(self):
        rs = RandomState(12345)
        x1 = rs.gumbel(size=50)
        A, crit, sig = stats.anderson(x1, 'gumbel_l')
        assert_array_less(crit[:-1], A)
        A, crit, sig = stats.anderson(x1, 'gumbel_r')
        assert_array_less(A, crit[-2:])
...
class TestMedianTest(TestCase):
    ...
    def test_bad_ties(self):
        assert_raises(
            ValueError, stats.median_test, [1, 2, 3], [4, 5], ties="foo")

    def test_bad_keyword(self):
        assert_raises(
            TypeError, stats.median_test, [1, 2, 3], [4, 5], foo="foo")
